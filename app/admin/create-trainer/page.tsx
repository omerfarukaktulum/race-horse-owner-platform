...
/**
 * Admin page to create a new trainer account
 * First step: Enter user's email and password
 * Then proceeds to trainer lookup from TJK
...
export default function CreateTrainerPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Navigate to trainer lookup (onboarding flow)
      router.push('/admin/create-trainer/trainer-lookup')
    } catch (error) {
      ...
      <div className="max-w-2xl mx-auto">
        <Card className="bg-white/90 backdrop-blur-sm shadow-xl border border-gray-200/50">
          <CardHeader className="text-center space-y-4">
            ...
            <div>
              <CardTitle className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-emerald-600 to-emerald-700">
                Yeni Antrenör Oluştur
              </CardTitle>
              <CardDescription className="text-gray-600 mt-2">
                Kullanıcının email ve şifresini girin, ardından TJK&apos;dan antrenör seçin
              </CardDescription>
            </div>
          </CardHeader>
          <CardContent>
