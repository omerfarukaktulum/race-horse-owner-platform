...
/**
 * Admin onboarding: Import horses page
 * Uses exact same design as original onboarding page
 */
export default function AdminImportHorsesPage() {
  ...
  const fetchHorses = async () => {
    try {
      // Get target user's owner profile
      const userResponse = await fetch('/api/admin/onboarding/target-user', {
        credentials: 'include',
      })
      
      const userData = await userResponse.json()

      if (!userResponse.ok || !userData.ownerProfile?.officialName) {
        throw new Error('Sahip bilgisi bulunamadı')
      }

      const ownerName = userData.ownerProfile.officialName
      const ownerRefValue = userData.ownerProfile.officialRef
      setOwnerRef(ownerRefValue)

      if (!ownerRefValue) {
        toast.error('TJK ID bulunamadı. Lütfen sahip bilgilerini kontrol edin.')
        setHorses([])
        setIsLoading(false)
        return
      }

      // Fetch horses using admin Playwright endpoint (for onboarding)
      ...
      // If API returns a message (e.g., no horses found during onboarding), show it
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      // Step 3: Redirect to completion page
      router.replace('/admin/create-owner/complete')
