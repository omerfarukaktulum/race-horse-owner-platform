...
/**
 * Admin onboarding: Stablemate setup page
 * Reuses the same UI as regular onboarding but binds to target user
 */
export default function AdminStablemateSetupPage() {
  const router = useRouter()
  const [name, setName] = useState('')
  const [foundationYear, setFoundationYear] = useState(new Date().getFullYear().toString())
  const [location, setLocation] = useState('')
  const [website, setWebsite] = useState('')
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [isLoadingUser, setIsLoadingUser] = useState(true)

  // Fetch target user's official name and pre-fill stablemate name
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        // Get target user info (from admin API)
        ...
        if (response.ok) {
          ...
          if (data.ownerProfile?.officialName) {
            // Pre-fill stablemate name with owner's official name
            setName(data.ownerProfile.officialName)
          }
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Use admin onboarding API which binds to target user
      const response = await fetch('/api/admin/onboarding/stablemate', {
        method: 'POST',
        headers: {
      ...
      router.push('/admin/create-owner/import-horses')
    } catch (error) {
